...
export interface SpecialModuleValidationResult {
  ...
  testType?: string;
  rowIndexes?: number[];
  fieldTarget?: 'testType' | 'moduleId' | 'manpowerDemand' | 'groupTotal';
}
...
export function validateSpecialModuleRows(
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (!row.testType) return { valid: false, errorCode: 'MODULE_GROUP_MISMATCH', testType: row.testType, rowIndexes: [index], fieldTarget: 'testType' };
    if (moduleId === undefined || !Number.isInteger(moduleId)) {
      return { valid: false, errorCode: 'SPECIAL_MODULE_REQUIRED', testType: row.testType, rowIndexes: [index], fieldTarget: 'moduleId' };
    }
    if (!Number.isFinite(manpowerDemand) || manpowerDemand === undefined || manpowerDemand <= 0 || Math.round(manpowerDemand * 10) !== manpowerDemand * 10 || manpowerDemand > SPECIAL_MODULE_MAX_MANPOWER) {
      return { valid: false, errorCode: 'SPECIAL_MODULE_MANPOWER_INVALID', testType: row.testType, rowIndexes: [index], fieldTarget: 'manpowerDemand' };
    }
    if (selectedModuleIds.has(moduleId)) {
      return { valid: false, errorCode: 'SPECIAL_MODULE_DUPLICATE', testType: row.testType, rowIndexes: rows.map((item, itemIndex) => item.moduleId === moduleId ? itemIndex : -1).filter((itemIndex) => itemIndex >= 0), fieldTarget: 'moduleId' };
    }
    selectedModuleIds.add(moduleId);
    const module = modules?.find((item) => item.id === moduleId);
    if (modules && !module) {
      return { valid: false, errorCode: 'MODULE_NOT_FOUND', testType: row.testType, rowIndexes: [index], fieldTarget: 'moduleId' };
    }
    ...
    if (!authoritativeTestType || !Object.prototype.hasOwnProperty.call(manpowerByTestType, authoritativeTestType) || manpowerByTestType[authoritativeTestType] <= 0) {
      return { valid: false, errorCode: 'MODULE_GROUP_MISMATCH', testType: authoritativeTestType, rowIndexes: [index], fieldTarget: 'testType' };
    }
    if (module && !module.enabled && row.historicalManpowerDemand !== manpowerDemand) {
      return { valid: false, errorCode: 'MODULE_DISABLED_FOR_NEW_DEMAND', testType: authoritativeTestType, rowIndexes: [index], fieldTarget: 'manpowerDemand' };
    }
  }
