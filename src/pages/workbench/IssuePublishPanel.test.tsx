  ...
  it('shows exact bucket gaps, reasons, group totals, and historical warning', () => {
    render(<IssuePublishPanel
      ...
      demands={[{
        ...
        manpowerSummary: [{
          testType: '功能测试', totalManpower: 5, specialManpower: 2, generalManpower: 3,
        }],
        specialModuleDemands: [{
          id: 501, demandId: 1001, moduleId: 11, moduleName: '支付模块',
          testType: '功能测试', enabled: true, manpowerDemand: 2,
          allocatedManpower: 1, remainingManpower: 1, createdAt: '', updatedAt: '',
        }],
      }]}
      fulfillment={[{
        demandId: 1001,
    ...
    expect(screen.getByText(/NO_QUALIFIED_STAFF/)).toHaveTextContent('没有熟悉支付模块且在周期内有容量的人员');
    ...
    expect(screen.getByText(/支付模块.*缺口 1/)).toBeInTheDocument();
    expect(screen.getByText(/功能测试通用人力.*缺口 2/)).toBeInTheDocument();
    expect(screen.getByText(/总量 5.*特殊 1.*通用 1/)).toBeInTheDocument();
    expect(screen.getByText('历史排班尚未完成人力归属')).toBeInTheDocument();
  });

  it('shows historical gating before a recommendation response exists', () => {
    render(<IssuePublishPanel
      conflicts={[]}
      onDismissConflicts={vi.fn()}
      fulfillment={[]}
    ...
    expect(screen.getByText(/历史项目.*历史排班尚未完成人力归属/)).toBeInTheDocument();
  });
});
